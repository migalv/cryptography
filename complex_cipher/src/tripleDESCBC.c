...
// Tamaño de la clave en bytes del Triple DES
#define TAM_CLAVE_BYTES_3D 24
// Número de claves de 64 bits
#define NUM_CLAVES 3
...
int main(int argc, char* argv[]) {
    ...
    int arg_index = 1;
    // 3 Claves de 64 bits cada una.
    uint64_t clave[3];
    // Array que contiene los 3 Arrays de claves de 56 bits para cada cifrado o descifrado
    ...
    // Puntero auxiliare para liberar memoria
    uint64_t* p_aux = NULL;
    // Determina el modo de cifrado del DES, CBC por defecto
    MODO modo = CBC;
    // Array de strings para parsear la clave
    char** strclave = NULL;
    ...
    if(argc == MIN_ARG && !strcmp(argv[arg_index],"-h")){
        ...
        printf("   └> -k Clave\t\t\tClave de 192 bits: 168 + 24 bits de paridad (es decir 42 Hexadecima)\n");
        printf("   └> -iv Vector\t\t\tVector de inicializacion (iv). (64 bits) (es decir 16 Hexadecima)\n");
		printf("\nOPCION\t\t\tDESCRIPCION\n");
        ...
        printf("\t%s -D -k \"89751A7928ECD735A60996AB83627CFDADCF34FA93\" -iv \"28ECD735A60996AB\" -i fichero_a_descifrar.txt -o fichero_descifrado.txt -m ECB\n", argv[0]);
        return 1;
    }
    ...
    // En caso de querer descifrar obtenemos la clave y el iv
		...
		if(!strcmp(argv[arg_index++], "-k")){
			if(strlen(argv[arg_index]) != 48){
				fprintf (stderr, "Error: El tamaño de la clave debe ser de 64 bits.\n");
		        return -1;
			}
            strclave = (char**)malloc(sizeof(char*)*NUM_CLAVES);

            for(i = 0; i < NUM_CLAVES; i++){
                strclave[i] = (char*)malloc(sizeof(char)*16);
                strncpy(strclave[i], argv[arg_index]+(i*16), 16);
                clave[i] = strtoull(strclave[i], NULL, 16);
                free(strclave[i]);
                // Comprobamos la paridad de clave para ver si es correcta
            	if(!check_parity(clave[i])){
    ...
    if(flag_cifrar){
        ...
        printf("Clave --> ");
        print_hexa(clave, 3, sizeof(uint64_t));

        printf("\nGenerando iv...\n");
        iv = generar_iv(clave[0]);
        printf("IV -->" );
    ...
    if(infilename){
        ...
        claves = (uint64_t**)malloc(sizeof(uint64_t*)*3);
        for(i = 0; i < NUM_CLAVES; i++){
            claves[i] = generar_claves(clave[i]);
        }
        ...
        while((nbytes = fread(buffer, sizeof(uint8_t), TAM_BUFFER, infile)) != 0){
            ...
            result = tripleDES(p_aux, nbytes, claves, iv, modo, flag_cifrar);
            free(p_aux);
            if(result == NULL){
        ...
        if(flag_cifrar){
            ...
            result = tripleDES(texto_b, nbytes, claves, iv, modo, flag_cifrar);
        }
        else{
            ...
            result = tripleDES(texto_b, nbytes*8, claves, iv, modo, flag_cifrar);
            nbytes*=8;
        }
